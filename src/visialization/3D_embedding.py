...
class Embedding3D(EnhancedVisualizer):
    ...
    def create_specialized_visualization(self, features, labels, plot_title, data_type, 
        ...
        if data_type == "storage":
            ...
            self.logger.warning(f"Unknown data type: {data_type}. Using generic preprocessing.")
        ...
        # Generate embedding based on chosen method - MODIFIED FOR 3D
        if method == 'tsne':
            ...
            self.logger.info(f"Applying 3D t-SNE for {data_type} data with perplexity={perplexity}, "
            ...
            try:
                ...
                pca = PCA(n_components=3, random_state=self.random_state)  # Changed to 3 for 3D
            ...
            try:
                ...
                self.logger.info(f"Applying 3D UMAP for {data_type} data with n_neighbors={n_neighbors}, min_dist={min_dist}")
            ...
            self.logger.info(f"Applying 3D PCA for {data_type} data")
        ...
        # Note: enhancement may need to be adjusted for 3D
        # embedding = self.enhance_separation_specialized(embedding, labels, data_type)
        
        # Create DataFrame for plotting
        df_viz = pd.DataFrame(embedding, columns=['Dim 1', 'Dim 2', 'Dim 3'])  # Added Dim 3
        df_viz['Label'] = labels
        ...
        if use_original_labels:
            ...
            if data_type == "memory":
                marker_size = 80  # Slightly smaller for 3D view
            elif data_type == "combined":
                marker_size = 75
            else:
                marker_size = 70
            ...
            for i, label in enumerate(unique_labels):
                ...
                ax.scatter(
                    ...
                    df_viz.loc[indices, 'Dim 3'],  # Added Dim 3
                ...
                if np.sum(indices) > 0:
                    ...
                    centroid_z = np.mean(df_viz.loc[indices, 'Dim 3'])  # Added Dim 3
                    ax.text(centroid_x, centroid_y, centroid_z, label, 
                            fontsize=10, fontweight='bold',  # Smaller font for 3D
                            ha='center', va='center')
        else:
        ...
        ax.set_title(f"{plot_title} ({data_type.capitalize()} Data) - 3D", fontsize=18, pad=20)
        ax.set_xlabel(f'{method_name} Dimension 1', fontsize=14)
        ax.set_ylabel(f'{method_name} Dimension 2', fontsize=14)
        ax.set_zlabel(f'{method_name} Dimension 3', fontsize=14)  # Added Dim 3 label
        ...
        if not use_original_labels or len(unique_labels) <= 15:
            ax.legend(title="Program Type" if not use_original_labels else "Program Name", 
                    fontsize=10, markerscale=1.2, loc='best')
        ...
        if method == 'tsne':
            if data_type == "memory":
                method_text = f"3D t-SNE: perplexity={perplexity}, learning_rate={learning_rate}, exaggeration={early_exaggeration}"
            elif data_type == "combined":
                method_text = f"3D t-SNE: perplexity={perplexity}, learning_rate={learning_rate}, max_iter={n_iter}"
            else:
                method_text = f"3D t-SNE: perplexity={perplexity}, learning_rate={learning_rate}, max_iter={n_iter}"
        elif method == 'umap':
            if data_type == "memory":
                method_text = f"3D UMAP: n_neighbors=10, min_dist=0.05"
            elif data_type == "combined":
                method_text = f"3D UMAP: n_neighbors=12, min_dist=0.08"
            else:
                method_text = f"3D UMAP: n_neighbors=15, min_dist=0.1"
        elif method == 'pca':
            ...
            if hasattr(self, 'pca_explained_variance'):
                ...
                var3 = self.pca_explained_variance[2] * 100  # Added Dim 3
                total_var = self.pca_total_explained_variance
                method_text = f"3D PCA: Dim1={var1:.1f}%, Dim2={var2:.1f}%, Dim3={var3:.1f}%, Total={total_var:.1f}%"
            else:
                method_text = "3D PCA: 3 components"
        else:
            method_text = f"3D {method.upper()}: standard parameters"
        ...
        ax.view_init(30, 45)  # Initial view angle
        
        # Use tight layout with adjusted parameters for 3D plot
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        
        if output_file:
            try:
                # For 3D plots, it's good to save multiple views
                base_name = str(output_file).replace('.png', '')
                
                # Save main view
                plt.savefig(output_file, dpi=300, bbox_inches='tight')
                
                # Save alternative views
                alt_views = [(20, 30), (20, 120), (20, 210), (20, 300)]
                for i, (elev, azim) in enumerate(alt_views):
                    ax.view_init(elev, azim)
                    plt.savefig(f"{base_name}_view{i+1}.png", dpi=300, bbox_inches='tight')
                
                self.logger.info(f"Saved 3D visualization with multiple views to {base_name}*.png")
            except Exception as e:
    ...
    def visualize_all_specialized(self, data_dict, output_dir=None, method='tsne'):
        ...
        try:
            self.logger.info(f"Creating 3D {method.upper()} visualization for storage features with program labels")
            storage_embedding, storage_df = self.create_specialized_visualization(
                data_dict["storage"],
                data_dict["original_labels"],
                f"3D {method.upper()} of Storage Features (Program Classification)",
                data_type="storage",
                output_file=output_dir / f"3d_{method}_storage_programs.png" if output_dir else None,
                use_original_labels=True,
                method=method
            )
        
            self.logger.info(f"Creating specialized 3D {method.upper()} visualization for memory features with program labels")
            memory_embedding, memory_df = self.create_specialized_visualization(
                data_dict["memory"],
                data_dict["original_labels"],
                f"3D {method.upper()} of Memory Features (Program Classification)",
                data_type="memory",
                output_file=output_dir / f"3d_{method}_memory_programs.png" if output_dir else None,
                use_original_labels=True,
                method=method
            )    
            self.logger.info(f"Creating specialized 3D {method.upper()} visualization for combined features with program labels")
            combined_embedding, combined_df = self.create_specialized_visualization(
                data_dict["combined"],
                data_dict["original_labels"],
                f"3D {method.upper()} of Combined Features (Program Classification)",
                data_type="combined",
                output_file=output_dir / f"3d_{method}_combined_programs.png" if output_dir else None,
                use_original_labels=True,
                method=method
            )
            self.logger.info(f"Creating specialized 3D {method.upper()} visualization for memory features with binary labels")
            self.create_specialized_visualization(
                data_dict["memory"],
                data_dict["binary_labels"],
                f"3D {method.upper()} of Memory Features (Binary Classification)",
                data_type="memory",
                output_file=output_dir / f"3d_{method}_memory_binary.png" if output_dir else None,
                use_original_labels=False,
                method=method
            )
           
            self.logger.info(f"Creating specialized 3D {method.upper()} visualization for storage features with binary labels")
            self.create_specialized_visualization(
                data_dict["storage"],
                data_dict["binary_labels"],
                f"3D {method.upper()} of Storage Features (Binary Classification)",
                data_type="storage",
                output_file=output_dir / f"3d_{method}_storage_binary.png" if output_dir else None,
                use_original_labels=False,
                method=method
            )
            
            # Combined with binary labels
            self.logger.info(f"Creating specialized 3D {method.upper()} visualization for combined features with binary labels")
            self.create_specialized_visualization(
                data_dict["combined"],
                data_dict["binary_labels"],
                f"3D {method.upper()} of Combined Features (Binary Classification)",
                data_type="combined",
                output_file=output_dir / f"3d_{method}_combined_binary.png" if output_dir else None,
                use_original_labels=False,
                method=method
            )
...
def main():
    ...
    try:
        ...
        # Add data statistics for debugging
        ...
        logger.info(f"Creating 3D {args.method.upper()} visualizations")
        visualizer = Embedding3D(
